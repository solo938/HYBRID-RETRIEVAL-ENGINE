...
Qdrant supports [multivector representations](/documentation/manage-data/vectors/index.md#multivectors) out of the box so that you can use any late interaction model as `ColBERT` or `ColPali` in Qdrant without any additional pre/post-processing.

This tutorial uses ColBERT as a first-stage retriever on a toy dataset.
You can see how to use ColBERT as a reranker in our [multi-stage queries documentation](/documentation/search/hybrid-queries/index.md#multi-stage-queries).
## Setup
...
Now, let's create a small [collection](/documentation/manage-data/collections/index.md) with our movie data.
For that, we will use the [multivectors](/documentation/manage-data/vectors/index.md#multivectors) functionality supported in Qdrant.
